/**
 * 🚀 LA JAMA - MONITOR LOGÍSTICO
 * Módulo Independiente de Carga Kinetic con Desenfoque de Entorno
 */
window.DespachoLoader = (function () {
    const OVERLAY_ID = 'jama-logistica-overlay';

    function lanzar(mensajeOpacional = "Calculando rutas óptimas...") {
        // Evitamos duplicidad en el DOM si ya está corriendo una instancia
        if (document.getElementById(OVERLAY_ID)) return;

        // 📝 Inyección de la estructura estructural con estilos inline controlados
        const HTML_LOADER = `
            <div id="${OVERLAY_ID}" class="despacho-blur-overlay">
                <div class="despacho-loader-container">
                    <!-- SVG Kinetic Original de Nawsome (Uiverse.io) -->
                    <svg class="pl-logistica" viewBox="0 0 160 160" width="160px" height="160px" xmlns="http://www.w3.org/2000/svg">
                        <defs>
                    ...
                    <span class="despacho-loading-text">${mensajeOpacional}</span>
                </div>
            </div>
        `;

        // Insertamos el nodo en caliente en el body
        document.body.insertAdjacentHTML('beforeend', HTML_LOADER);

        // ⏱️ AUTO-DESTRUCCIÓN TRANSACCIONAL (Máximo 1 segundo en pantalla)
        setTimeout(() => {
            const overlayNode = document.getElementById(OVERLAY_ID);
            if (overlayNode) {
                overlayNode.classList.add('fade-out-clean');
                // Esperamos que termine la transición css para limpiar el DOM
                overlayNode.addEventListener('animationend', () => overlayNode.remove());
            }
        }, 500);
    }
